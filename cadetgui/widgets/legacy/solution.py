    ...
    try:
        ...
        with out:
            ...
            if callable(plot_attr):
                obj = plot_attr()
                # Try to display the current figure if one was created
                try:
                    display(plt.gcf())
                except Exception:
                    pass
...
class SolutionWidget(BaseWidget):
    ...
    def __init__(
        ...
        *,
        ...
        # Separate outputs: one for the plot, one for logs/stdout/stderr
        self._plot_out = W.Output()
        self._log_out = W.Output()
        ...
        # Track a single live figure/axes for reuse when possible
        self._fig = None
        self._ax = None
    ...
    # ----- backend helper -----
    def _enable_widget_backend(self) -> None:
        """Try to enable ipympl widget backend for interactive plots."""
        try:
            import ipympl  # noqa: F401  # registers the widget backend if installed
            import matplotlib.pyplot as plt
            try:
                plt.switch_backend("module://ipympl.backend_nbagg")
            except Exception:
                # May already be using a compatible interactive backend (or switch unsupported here).
                pass
        except Exception:
            # ipympl not installed; fall back to whatever backend is configured.
            pass
    ...
    def _build_body(self) -> W.Widget:
        ...
        return W.VBox(
            [
                ...
                self._plot_out,  # plot area (single live figure)
                self._log_out,   # logs/stdout/stderr
            ]
        )
    ...
    def _on_run(self, _btn: W.Button) -> None:
        ...
        try:
            ...
            if self._render_field_ui_if_applicable(result):
                ...
                with self._log_out:
                    print(repr(result))
                _try_plot_result_generic(result, self._log_out)
    ...
    def _on_result_key_changed(self) -> None:
        ...
        if self._render_field_ui_if_applicable(val):
            ...
            with self._log_out:
                print(repr(val))
            _try_plot_result_generic(val, self._log_out)
            self.set_status(f"<em>Viewing: {key}</em>")
    ...
    def _render_field_ui_if_applicable(self, result: Any) -> bool:
        ...
        # reset control registry for fresh UI
        ...
        showed_component_selector = False
        if components is not None and len(components) > 0:
            ...
            showed_component_selector = True

        # Slicing controls
        coords = _coords_with_units(field)
        slicer_boxes: list[W.Widget] = []
        numeric_axis_names: list[str] = []

        for name, (grid, unit) in coords.items():
            # If we've already provided a dedicated components selector, skip duplicating
            if name == "component" and showed_component_selector:
                continue

            # ensure numpy array for dtype inspection
            arr = np.asarray(grid)
            is_numeric = np.issubdtype(arr.dtype, np.number)

            ulabel = f"[{unit}]" if unit else ""
            if is_numeric:
                # numeric axis → checkbox + FloatText + method dropdown
                try:
                    v0 = float(arr[0]) if arr.size else 0.0
                ...
                numeric_axis_names.append(name)

            else:
                # categorical axis → multi-select
                options = [str(x) for x in arr.tolist()]
                ...
                self._field_controls[f"sel_{name}"] = sel
                sel.observe(lambda _ch: self._plot_field_from_controls(), names="value")
                slicer_boxes.append(sel)

        if slicer_boxes:
            rows.append(W.VBox([W.HTML("<b>Slicing</b>"), *slicer_boxes]))

        # Interpolation controls (numeric axes only)
        if numeric_axis_names:
            interp_enable = W.Checkbox(description="Interpolate", value=False)
            interp_axis = W.Dropdown(
                description="along",
                options=numeric_axis_names,
                value=numeric_axis_names[0],
            )
            ...
            # No numeric axes → disable interpolation UI entirely
            interp_note = W.HTML("<i>No numeric axes available for interpolation.</i>")
            rows.append(W.VBox([W.HTML("<b>Interpolation</b>"), interp_note]))

        # Calculus controls (numeric axes only)
        if numeric_axis_names:
            ...
            calc_note = W.HTML("<i>No numeric axes available for analysis.</i>")
            rows.append(W.VBox([W.HTML("<b>Analysis</b>"), calc_note]))

        # Plot button (manual trigger in addition to reactive updates)
    ...
    def _plot_field_from_controls(self) -> None:
        # Make the figure area behave like a single live canvas
        self._plot_out.clear_output(wait=True)

        # Enable interactive backend if possible (ipympl)
        self._enable_widget_backend()
        ...
        # Draw a single figure and show it once in the plot output
        try:
            with self._plot_out:
                ax = None
                # Try to reuse an existing axes if Field.plot supports it
                ...
                if ax is not None:
                    # Many APIs accept 'ax=...'; try first
                    try:
                        ret = fld3.plot({}, ax=ax)
                    except TypeError:
                        # fallback: set current axes and let plot create artists
                        plt.sca(ax)
                        ret = fld3.plot({})
                else:
                    # first time (or reuse failed): make a fresh figure
                    fig, ax = plt.subplots()
                    ret = fld3.plot({})
                    self._fig, self._ax = fig, ax

                # Capture figure reference if plot returned axes
                try:
                    if hasattr(ret, "get_figure"):
                        self._fig = ret.get_figure()
                        try:
                            # if we didn’t create ax above, pull one from ret
                            self._ax = ret if hasattr(ret, "plot") else getattr(ret, "axes", [None])[0]
                        except Exception:
                            pass
                    else:
                        # last-resort: current fig/ax
                        self._fig = plt.gcf()
                        self._ax = plt.gca()
                except Exception:
                    pass

                # Display exactly once (plot_out was cleared above)
                display(self._fig)

        except Exception:
            # very defensive fallback (generic plot/print)
            with self._plot_out:
                try:
                    ...
                    ret = fld3.plot(fixed)
                    try:
                        self._fig = ret.get_figure() if hasattr(ret, "get_figure") else plt.gcf()
                        display(self._fig)
                    except Exception:
                        pass
                except Exception:
                    _try_plot_result_generic(fld3, self._log_out)
